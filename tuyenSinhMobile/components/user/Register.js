...
const Register = ({ navigation }) => {
    ...
    const uploadImageToCloudinary = async (uri) => {
        try {
            ...
            console.log('Cloudinary response:', result); // Log the entire response for debugging

            if (response.ok) {
                // Extract the image path from the secure_url
    ...
    const register = async () => {
        ...
        try {
            ...
            if (!avatarPath) {
                return; // If avatarPath is null, exit the function
            }
            ...
            formData.append('avatar', avatarPath); // Only the path, not the full URL
            ...
            // Dispatch with only the path for the avatar
            dispatch({ type: "login", payload: { token, ...user, avatar: avatarPath } });
